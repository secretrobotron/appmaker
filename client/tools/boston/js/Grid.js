...
Grid.prototype.addDynamicPoints = function () {
  ...
  if ($('#BostonStageBody #body').children('.scrap:not(.selection)').length > 200)
    return true
  $('#BostonStageBody #body').children('.scrap:not(.selection)').each(function(){
...
Grid.prototype.addFixedPoints = function () {
  ...
  // verticals
  var start = $('#BostonStageBody').position().left
  var end = $('#BostonStageBody').position().left + $('#BostonStageBody').outerWidth()
  this.horizontalSpacing = parseFloat(this.horizontalSpacing)
  for (var i = start; i <= end; i = i + this.horizontalSpacing) {
    this.addPoint(i, 0, '#BostonStageBody')  
  }
  
  start = $('#BostonStageBody').position().top
  end = $('#BostonStageBody').position().top + $('#BostonStageBody').outerHeight()
  this.verticalSpacing = parseFloat(this.verticalSpacing)
  for (var i = start; i <= end; i = i + this.verticalSpacing) {
    this.addPoint(0, i, '#BostonStageBody')  
  }
...
//    this.points.clear()
  ...
  if (this.snapToContainer) {
    ...
    // left
//    this.addPoint(0, $('#BostonStageBody').position().top, '#BostonStageBody')
    // right
    this.addPoint($('#BostonStageBody').outerWidth(), 0, '#BostonStageBody')
    // center
    this.addPoint(Math.round($('#BostonStageBody').outerWidth()/2), 0, '#BostonStageBody')
    // top
    this.addPoint(0, 0, '#BostonStageBody')
    // bottom
    this.addPoint(0, $('#BostonStageBody').height(), '#BostonStageBody')
    // middle
    this.addPoint(0, Math.round($(window).height()/2), '#BostonStageBody')
...
Grid.prototype.draw = function () {
  ...
  for (var i in this.points) {
    ...
    if (point.selector !== '#BostonStageBody')
      continue
...
//  console.log(point2.selector)
  ...
  // Dont draw lines based on objects
  if (point2.selector !== '#BostonStageBody')
...
Grid.prototype.drawSnaplineCanvas = function () {
  ...
  this.width = $("#BostonStageBody").width()
  this.height = $("#BostonStageBody").height()
