...
Scrap.prototype.edit = function (selectAll) {
  ...
  // Default block editor
    ...
    Chopsticks.contentEditor.focus(this.selector(), selectAll)
...
//  this.div.attr('page', Chopsticks.stage.activePage)
...
Scrap.prototype.render = function (index) {
  ...
  if (tag && tag.match(/style|link/)) {
    ...
    this.setProperties()
    this.setContent()
    $('#ChopsticksStageHead').append(this.div.toHtml())
    ...
    if (tag === 'style' && this.get('content')) {
      ...
      bodyProperties.forEach(function (property, index) {
        ...
        if (!val)
          return true
        $('#ChopsticksStageBody').css(property, val)
      })
    }
  ...
  if (tag && tag === 'body') {
    ...
    $('#ChopsticksStageBody').append(html)
    return this
  }
...
Scrap.prototype.selector = function () {
  ...
  if (!selector)
    return ''
  return '#ChopsticksStageBody>#' + selector
}
...
Scrap.selectOnTap =  function (event) {
  ...
  if (Chopsticks.Mouse.down && Chopsticks.Mouse.down.altKey) {
    if (!$(this).hasClass('selection') && $('.selection').length) {
      Chopsticks.stage.selection.nest($(this).attr('path'))
      return false
    }
  }
