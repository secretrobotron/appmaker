...
h.isArray(d)&&(p=d[1],d=b[q]=d[0]),q!==k&&(b[k]=d,delete b[q]),m=h.cssHooks[k],m&&"expand"in m)for(q in d=m.expand(d),delete b[k],d)q in b||(b[q]=d[q],c[q]=p);else c[k]=p;for(;g>f;f++)if(d=Nb[f].call(l,a,b,l.opts))return d;var y=l;h.each(b,function(a,b){for(var K=(Eb[a]||[]).concat(Eb["*"]),c=0,d=K.length;d>c&&!K[c].call(y,a,b);c++);});return h.isFunction(l.opts.start)&&l.opts.start.call(a,l),h.fx.timer(h.extend(u,{elem:a,anim:l,queue:l.opts.queue})),l.progress(l.opts.progress).done(l.opts.done,l.opts.complete).fail(l.opts.fail).always(l.opts.always)}
...
kd=/ jQuery\d+="(?:null|\d+)"/g,Cc=RegExp("<(?:"+qc+")[\\s/>]","i"),jc=/^\s+/,Dc=/<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi,Ec=/<([\w:]+)/,Fc=/<tbody/i,ld=/<|&#?\w+;/,md=/<(?:script|style|link)/i,ac=/^(?:checkbox|radio)$/i,nd=/checked\s*(?:[^=]|=\s*.checked.)/i,Gc=/^$|\/(?:java|ecma)script/i,Sc=/^true\/(.*)/,od=/^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)>\s*$/g,Ia={option:[1,"<select multiple='multiple'>","</select>"],legend:[1,"<fieldset>","</fieldset>"],area:[1,"<map>","</map>"],
...
w(d,!u&&l(a,"script")),f},buildFragment:function(a,b,c,d){for(var e,f,g,j,u,k,q,m=a.length,p=r(b),y=[],n=0;m>n;n++)if(f=a[n],f||0===f)if("object"===h.type(f))h.merge(y,f.nodeType?[f]:f);else if(ld.test(f)){j=j||p.appendChild(b.createElement("div"));u=(Ec.exec(f)||["",""])[1].toLowerCase();q=Ia[u]||Ia._default;j.innerHTML=q[1]+f.replace(Dc,"<$1></$2>")+q[2];for(e=q[0];e--;)j=j.lastChild;if(!h.support.leadingWhitespace&&jc.test(f)&&y.push(b.createTextNode(jc.exec(f)[0])),!h.support.tbody)for(e=(f="table"!==
...
c,d,e){if(a&&3!==a.nodeType&&8!==a.nodeType&&a.style){var f,g,j,l=h.camelCase(c),u=a.style;if(c=h.cssProps[l]||(h.cssProps[l]=A(u,l)),j=h.cssHooks[c]||h.cssHooks[l],d===b)return j&&"get"in j&&(f=j.get(a,!1,e))!==b?f:u[c];if(g=typeof d,"string"===g&&(f=sd.exec(d))&&(d=(f[1]+1)*f[2]+parseFloat(h.css(a,c)),g="number"),!(null==d||"number"===g&&isNaN(d)||("number"!==g||h.cssNumber[l]||(d+="px"),h.support.clearCloneStyle||""!==d||0!==c.indexOf("background")||(u[c]="inherit"),j&&"set"in j&&(d=j.set(a,d,
...
h.each({margin:"",padding:"",border:"Width"},function(a,c){h.cssHooks[a+c]={expand:function(b){var d=0,e={};for(b="string"==typeof b?b.split(" "):[b];4>d;d++)e[a+ab[d]+c]=b[d]||b[d-2]||b[0];return e}};Hc.test(a)||(h.cssHooks[a+c].set=m)});var ud=/%20/g,Uc=/\[\]$/,Jc=/\r?\n/g,vd=/^(?:submit|button|image|reset|file)$/i,wd=/^(?:input|select|textarea|keygen)/i;h.fn.extend({serialize:function(){return h.param(this.serializeArray())},serializeArray:function(){return this.map(function(){var a=h.prop(this,
...
arguments.length?this.on(c,null,a,b):this.trigger(c)}});h.fn.hover=function(a,c){return this.mouseenter(a).mouseleave(c||a)};var jb,$a,mc=h.now(),nc=/\?/,xd=/#.*$/,Kc=/([?&])_=[^&]*/,yd=/^(.*?):[ \t]*([^\r\n]*)\r?$/gm,zd=/^(?:GET|HEAD)$/,Ad=/^\/\//,Lc=/^([\w.+-]+:)(?:\/\/([^\/?#:]*)(?::(\d+)|)|)/,Mc=h.fn.load,Nc={},bc={},Oc="*/".concat("*");try{$a=oa.href}catch(Gd){$a=J.createElement("a"),$a.href="",$a=$a.href}jb=Lc.exec($a.toLowerCase())||[];h.fn.load=function(a,c,d){if("string"!=typeof a&&Mc)return Mc.apply(this,
...
(c=a,a=b);c=c||{};var e,f,g,j,l,u,k,q,m=h.ajaxSetup({},c),p=m.context||m,y=m.context&&(p.nodeType||p.jquery)?h(p):h.event,n=h.Deferred(),S=h.Callbacks("once memory"),C=m.statusCode||{},fa={},w={},A=0,x="canceled",R={readyState:0,getResponseHeader:function(a){var c;if(2===A){if(!q)for(q={};c=yd.exec(j);)q[c[1].toLowerCase()]=c[2];c=q[a.toLowerCase()]}return null==c?null:c},getAllResponseHeaders:function(){return 2===A?j:null},setRequestHeader:function(a,c){var b=a.toLowerCase();return A||(a=w[b]=w[b]||
a,fa[a]=c),this},overrideMimeType:function(a){return A||(m.mimeType=a),this},statusCode:function(a){var c;if(a)if(2>A)for(c in a)C[c]=[C[c],a[c]];else R.always(a[R.status]);return this},abort:function(a){a=a||x;return k&&k.abort(a),d(0,a),this}};if(n.promise(R).complete=S.add,R.success=R.done,R.error=R.fail,m.url=((a||m.url||$a)+"").replace(xd,"").replace(Ad,jb[1]+"//"),m.type=c.method||c.type||m.method||m.type,m.dataTypes=h.trim(m.dataType||"*").toLowerCase().match(ya)||[""],null==m.crossDomain&&
(e=Lc.exec(m.url.toLowerCase()),m.crossDomain=!(!e||e[1]===jb[1]&&e[2]===jb[2]&&(e[3]||("http:"===e[1]?80:443))==(jb[3]||("http:"===jb[1]?80:443)))),m.data&&m.processData&&"string"!=typeof m.data&&(m.data=h.param(m.data,m.traditional)),Q(Nc,m,c,R),2===A)return R;(u=m.global)&&0===h.active++&&h.event.trigger("ajaxStart");m.type=m.type.toUpperCase();m.hasContent=!zd.test(m.type);g=m.url;m.hasContent||(m.data&&(g=m.url+=(nc.test(g)?"&":"?")+m.data,delete m.data),!1===m.cache&&(m.url=Kc.test(g)?g.replace(Kc,
...
b});var Cb,Lb,Bd=0,pc=a.ActiveXObject&&function(){for(var a in Cb)Cb[a](b,!0)};h.ajaxSettings.xhr=a.ActiveXObject?function(){var c;if(!(c=!this.isLocal&&U()))a:{try{c=new a.ActiveXObject("Microsoft.XMLHTTP");break a}catch(b){}c=void 0}return c}:U;Lb=h.ajaxSettings.xhr();h.support.cors=!!Lb&&"withCredentials"in Lb;(Lb=h.support.ajax=!!Lb)&&h.ajaxTransport(function(c){if(!c.crossDomain||h.support.cors){var d;return{send:function(e,f){var g,j,l=c.xhr();if(c.username?l.open(c.type,c.url,c.async,c.username,
...
"string"==typeof l.responseText&&(q.text=l.responseText);try{k=l.statusText}catch(m){k=""}j||!c.isLocal||c.crossDomain?1223===j&&(j=204):j=q.text?200:404}}catch(p){e||f(-1,p)}q&&f(j,k,q,u)};c.async?4===l.readyState?setTimeout(d):(g=++Bd,pc&&(Cb||(Cb={},h(a).unload(pc)),Cb[g]=d),l.onreadystatechange=d):d()},abort:function(){d&&d(b,!0)}}}});var kb,Zb,Cd=/^(?:toggle|show|hide)$/,Dd=RegExp("^(?:([+-])=|)("+Ba+")([a-z%]*)$","i"),Ed=/queueHooks$/,Nb=[function(a,c,b){var d,e,f,g,j,l,u=this,k=a.style,q={},
...
m[e],g=u.createTween(d,p?f[d]:0),q[d]=f[d]||h.style(a,d),d in f||(f[d]=g.start,p&&(g.end=g.start,g.start="width"===d||"height"===d?1:0))}}],Eb={"*":[function(a,c){var b,d,e=this.createTween(a,c),f=Dd.exec(c),g=e.cur(),j=+g||0,l=1,u=20;if(f){if(b=+f[2],d=f[3]||(h.cssNumber[a]?"":"px"),"px"!==d&&j){j=h.css(e.elem,a,!0)||b||1;do l=l||".5",j/=l,h.style(e.elem,a,j+d);while(l!==(l=e.cur()/g)&&1!==l&&--u)}e.unit=d;e.start=j;e.end=f[1]?j+(f[1]+1)*b:b}return e}]};h.Animation=h.extend(H,{tweener:function(a,
...
d,e,f){function g(a){a.stopPropagation&&a.stopPropagation();a.preventDefault&&a.preventDefault();a.returnValue=!1}function l(a){if(k){if(r&&9>document.documentMode&&!a.button)return j();var b=a.originalEvent.touches,e=b?b[0].pageY:a.pageY,b=Math.max(0,Math.min((b?b[0].pageX:a.pageX)-m.left,n)),e=Math.max(0,Math.min(e-m.top,p));w&&g(a);d.apply(c,[b,e,a])}}function j(){k&&(b(q).unbind(x),b(q.body).removeClass("sp-dragging"),f.apply(c,arguments));k=!1}d=d||function(){};e=e||function(){};f=f||function(){};
var q=c.ownerDocument||document,k=!1,m={},p=0,n=0,w="ontouchstart"in a,x={};x.selectstart=g;x.dragstart=g;x[w?"touchmove":"mousemove"]=l;x[w?"touchend":"mouseup"]=j;b(c).bind(w?"touchstart":"mousedown",function(a){if(!(a.which?3==a.which:2==a.button)&&!k&&!1!==e.apply(c,arguments))k=!0,p=b(c).height(),n=b(c).width(),m=b(c).offset(),b(q).bind(x),b(q.body).addClass("sp-dragging"),w||l(a),g(a)})}var n={beforeShow:f,move:f,change:f,show:f,hide:f,color:!1,flat:!1,showInput:!0,showButtons:!1,clickoutFiresChange:!1,
...
if(!TextPrompt.onclose)return null;TextPrompt.onclose();TextPrompt.onclose=null},open:function(a,b,d){TextPrompt.callback=d;$(".TextPrompt").show();$("#TextPromptTextarea").val(b).focus()}};$(document).on("ready",TextPrompt.load);var QRCode;
...
function(a){a.stopPropagation();return!1});$("#DesignerBar").on("slide slidestart",function(a){a.stopPropagation()})});Designer.codePanel={};Designer.codePanel.livePreview=!1;Designer.codePanel.livePreviewTimeout=!1;Designer.codePanel.livePreviewStart=function(){clearTimeout(Designer.codePanel.livePreviewTimeout);Designer.codePanel.livePreviewTimeout=setTimeout("Designer.codePanel.livePreview()",500)};
Designer.codePanel.livePreview=function(){var a=new Space($("#DesignerCodePanel").val());Designer.stage.selection.exists()&&Designer.stage.selection.clear();Designer.page=new Page(a);Designer.stage.render()};Designer.codePanel.close=function(){$("#DesignerCodePanel").hide();$("#DesignerStage").css("padding-left",Designer.codePanel.currentPadding);Designer.off("selection",Designer.codePanel.load);Designer.off("stage",Designer.codePanel.load)};Designer.codePanel.isOpen=function(){return 0<$("#DesignerCodePanel:visible").length};
Designer.codePanel.load=function(){$("#DesignerCodePanel").val(Designer.page.toString())};
Designer.codePanel.open=function(){var a=$("#DesignerCodePanel");a.show();Designer.codePanel.currentPadding=$("#DesignerStage").css("padding-left");$("#DesignerStage").css("padding-left","40%");Designer.codePanel.load();a.on("keyup",Designer.codePanel.livePreviewStart);a.on("blur",Designer.stage.commit);a.on("tap mousedown click slide slidestart slideend mouseup",function(a){a.stopPropagation()});Designer.on("selection",Designer.codePanel.load);Designer.on("stage",Designer.codePanel.load)};
Designer.codePanel.toggle=function(){Designer.codePanel.isOpen()?Designer.codePanel.close():Designer.codePanel.open()};Designer.dragComponent=function(){var a=$(this).attr("path");Designer.stage.dragAndDrop(Designer.components.get(a));mixpanel.track("I dragged a component")};
...
Designer.menu.nextName=function(a){a=a||"untitled";if(!(a in Project.values.pages.values))return a;for(var b=1;1E3>b;b++)if(!(a+b in Project.values.pages.values))return a+b};Designer.menu.prettyPrint=!0;Designer.menu.publish=function(a){var b=Project.get("pages "+a).toString(),b=(new Page(b)).toHtml(function(){return"true"===this.get("draft")?"":this.div.toHtml()});Designer.menu.prettyPrint&&(b=html_beautify(b));Explorer.set(a+".html",b)};
...
Designer.onpaste=function(a){if($("input:focus, div:focus, textarea:focus, a:focus").length)return!0;var b=void 0;a.clipboardData&&a.clipboardData.getData&&(b=a.clipboardData.getData("text/plain"));a="scraps";b.match(/^\s*\</)&&(a="html");"scraps"===a?Designer.pasteScraps(b):"html"===a?Designer.pasteScraps($.htmlToScraps(b)):"css"===a&&Designer.pasteCss(b);mixpanel.track("I pasted something")};
...
$.fn.scrollMinimal=function(a){var b=this.position().top,d=this.outerHeight(!0),c=$("#DesignerStage").scrollTop(),e=$("#DesignerStage").height();b<c?a?$("#DesignerStage").animate({scrollTop:b},"slow","swing"):$("#DesignerStage").scrollTop(b):b+d>c+e&&(a?$("#DesignerStage").animate({scrollTop:b-e+d},"slow","swing"):$("#DesignerStage").scrollTop(b-e+d))};Designer.stage.selection.saved=[];
Designer.stage.selection.alignLeft=function(){var a;$(".selection").each(function(){if(!a||$(this).position().left<a)a=$(this).position().left});Designer.stage.selection.css("left "+a+"px")};Designer.stage.selection.alignRight=function(){var a;$(".selection").each(function(){if(!a||$(this).position().left>a)a=$(this).position().left});Designer.stage.selection.css("left "+a+"px")};
Designer.stage.selection.alignTop=function(){var a;$(".selection").each(function(){if(!a||$(this).position().top<a)a=$(this).position().top});Designer.stage.selection.css("top "+a+"px")};Designer.stage.selection.alignBottom=function(){var a;$(".selection").each(function(){if(!a||$(this).position().top>a)a=$(this).position().top});Designer.stage.selection.css("top "+a+"px")};
Designer.stage.selection.boxShadow=function(a){$(".selection").each(function(){var b=$(this).scrap();1>a?(b.set("style box-shadow","none"),$(this).css("box-shadow","none")):(b.set("style box-shadow","0px 1px "+a+"px #888"),$(this).css("box-shadow","0px 1px "+a+"px #888"))})};Designer.stage.selection.capture=function(){Designer.stage.selection.captured=Designer.stage.selection.toSpace()};
...
Designer.stage.selection.cssPreview=function(a){if(!a)return!1;a=new Space(a);$(".selection").each(function(){var b=$(this).scrap().get("style");b||($(this).scrap().set("style",new Space),b=$(this).scrap().get("style"));b.patch(a);$(this).css(a.values)})};Designer.stage.selection.cssPrompt=function(){var a=prompt("Enter a CSS command like font-family Arial","");a&&Designer.stage.selection.css(a)};
Designer.stage.selection.delete=function(){$(".selection").each(function(){var a=$(this).scrap();$(this).deselect(!0).remove();a&&Designer.page.delete(a.getPath())})};
Designer.stage.selection.distributeVertical=function(){var a=_.sortBy($(".selection"),function(a){return $(a).position().top}),b=0,d=0;_.each(a,function(a,c,k){if(0===c)return!0;c=k[c-1];$(a).attr("id");b+=$(a).position().top-($(c).position().top+$(c).outerHeight());d++});var c=Math.floor(b/d);0>c&&(c=0);_.each(a,function(a,b,d){if(0===b)return!0;b=d[b-1];d=$(a).scrap();d.set("style top",$(b).position().top+$(b).outerHeight()+c+"px");$(a).css("top",d.get("style top"))});$(".handle").trigger("update");
Designer.stage.commit()};
Designer.stage.selection.distributeHorizontal=function(){var a=_.sortBy($(".selection"),function(a){return $(a).position().left}),b=0,d=0;_.each(a,function(a,c,k){if(0===c)return!0;c=k[c-1];$(a).attr("id");b+=$(a).position().left-($(c).position().left+$(c).outerWidth());d++});var c=Math.floor(b/d);0>c&&(c=0);_.each(a,function(a,b,d){if(0===b)return!0;b=d[b-1];d=$(a).scrap();d.set("style left",$(b).position().left+$(b).outerWidth()+c+"px");$(a).css("left",d.get("style left"))});$(".handle").trigger("update");
Designer.stage.commit();Flasher.success("Distributed",1E3)};Designer.stage.selection.duplicate=function(){$(".selection").each(function(){$(this).duplicate()});Designer.stage.commit()};
Designer.stage.selection.editLoop=function(){var a=prompt("What property do you want to edit?");if(!a)return!1;$(".selection");$(".selection").each(function(){var b=$(this).scrap();$(this).addClass("DesignerHighlightedScrap");$(this).scrollMinimal();var d=b.get(a),c=prompt("Set "+a+" of "+id+" to",d);if(!c)return!0;if(c==d)return $(this).removeClass("DesignerHighlightedScrap"),!0;$(this).removeClass("DesignerHighlightedScrap");b.set(a,c);b.render()});Designer.stage.commit()};
Designer.stage.selection.editProperty=function(){var a=$(".selection").scrap(),b=prompt("What property do you want to edit?","");if(!b)return!1;var d=a.get(b);TextPrompt.open("Enter new value...",d.toString(),function(c){a.set(b,c);Designer.stage.commit();Designer.stage.open(Designer.stage.activePage)})};Designer.stage.selection.editSource=function(){Designer.stage.selection.capture();Designer.stage.selection.save();TextPrompt.open("Enter code...",Designer.stage.selection.captured.toString(),Designer.stage.selection.modify)};
Designer.stage.selection.exists=function(){return $(".selection").length};Designer.stage.selection.modify=function(a){a=new Space(a);Designer.page.patch(Designer.stage.selection.captured.diff(a));Designer.stage.commit();Designer.stage.open(Designer.stage.activePage);Designer.stage.selection.restore()};
...
Designer.stage.selection.patch=function(a){"string"===typeof a&&(a=new Space(a));$(".selection").each(function(){var b=$(this).scrap();$(this).deselect();b.patch(a);$(this).replaceWith(b.toHtml(Scrap.devFilter));b.element().selectMe()});Designer.stage.commit()};Designer.stage.selection.patchPrompt=function(){var a=prompt("Enter a patch like content hi","");a&&Designer.stage.selection.patch(a)};
Designer.stage.selection.renameScraps=function(){var a=$(".selection").length;$(".selection").each(function(b){var d=$(this).scrap();$(this).addClass("DesignerHighlightedScrap");$(this).scrollMinimal();b=prompt("Renaming block "+(b+1)+"/"+a+". Enter a new ID",d.id);if(b==d.id)return $(this).removeClass("DesignerHighlightedScrap"),!0;if(!b)return!0;var c=new Scrap(b,d.toString());Designer.page.set(b,c);$(this).deselect().remove();Designer.page.delete(d.getPath());c.render();c.element().selectMe()});
Designer.stage.commit()};Designer.stage.selection.restore=function(){for(var a in Designer.stage.selection.saved){var b=Designer.stage.selection.saved[a];$(b).length&&$(b).selectMe(!0)}Designer.trigger("selection")};Designer.stage.selection.save=function(){Designer.stage.selection.saved=[];$(".selection").each(function(){Designer.stage.selection.saved.push($(this).scrap().selector())})};
Designer.stage.selection.toSpace=function(){var a=new Space;$(".selection").each(function(){var b=$(this).scrap();a.set(b.getPath(),new Space(b.toString()))});return a};Designer.broadcastSelection=function(a){var b=a||"",d="";$(".selection").each(function(){$(this).scrap()&&(b+=d+$(this).scrap().selector(),d=",")});b+="{box-shadow: 0 0 4px "+Screen.get("color")+";cursor: not-allowed;}";Screen.set("selection",b)};
...
Designer.stage.close=function(){$("#DesignerStageBody").attr("style","");$("#DesignerStageHead").html("");$("#DesignerRemoteSelections").html("");$(".scrap,#body").remove()};
...
function(a){a.preventDefault()});var a=store.get("activePage")||"index";Project.get("pages "+a)||(a="index");Project.get("pages "+a)||Designer.menu.create("index");Designer.stage.open(a);navigator.userAgent.match(/iPad|iPhone|iPod/i)||($(document).on("touchstart",Designer.stopPropagation),$(document).on("touchmove",Designer.preventDefault));window.addEventListener("copy",Designer.oncopy,!1);window.addEventListener("cut",Designer.oncut,!1);window.addEventListener("paste",Designer.onpaste,!1);window.addEventListener("resize",
Designer.onresize,!1);$("body").on("keydown",Designer.onkeydown);Events.shortcut.shortcuts=Designer.shortcuts;navigator.userAgent.match(/(iPad|iPhone|iPod)/i)?(document.addEventListener("touchstart",Designer.Mouse.onTouchStart,!0),document.addEventListener("touchend",Designer.Mouse.onTouchEnd,!0),document.addEventListener("touchmove",Designer.Mouse.onTouchMove,!0)):(document.addEventListener("mousedown",Designer.Mouse.onmousedown,!0),document.addEventListener("mousemove",Designer.Mouse.onmousemove,
!0),document.addEventListener("mouseup",Designer.Mouse.onmouseup,!0));Designer.isFirstOpen&&(Designer.isFirstOpen=!1,Designer.trigger("firstOpen"));$("#DesignerComponentsTab").trigger("click")});
...
Designer.on("commit",Designer.stage.expand);Designer.on("page",function(){Designer.stage.expand();Designer.stage.views.get(Designer.stage.currentView)();$("#DesignerStageBody").width();Designer.stage.reset()});Designer.trackShortcuts=function(a){mixpanel.track("I used the Designer keyboard shortcut "+a)};Designer.shortcuts={};Designer.shortcuts["meta+shift+p"]=Designer.stage.selection.patchPrompt;Designer.shortcuts["ctrl+a"]=Designer.stage.selectAll;Designer.shortcuts["meta+a"]=Designer.stage.selectAll;
Designer.shortcuts["meta+p"]=function(){window.open(Designer.stage.activePage+".html?"+(new Date).getTime(),"published")};Designer.shortcuts["meta+shift+left"]=Designer.stage.selection.alignLeft;Designer.shortcuts["meta+shift+right"]=Designer.stage.selection.alignRight;Designer.shortcuts["meta+shift+up"]=Designer.stage.selection.alignTop;Designer.shortcuts["meta+shift+down"]=Designer.stage.selection.alignBottom;Designer.shortcuts["meta+shift+v"]=Designer.stage.selection.distributeVertical;
Designer.shortcuts["meta+shift+h"]=Designer.stage.selection.distributeHorizontal;Designer.shortcuts["shift+d"]=Designer.stage.selection.distributeHorizontal;Designer.shortcuts["alt+o"]=Explorer.quickEdit;Designer.deleteShortcut=function(){Designer.stage.selection.delete();Designer.stage.commit()};Designer.shortcuts["delete"]=Designer.deleteShortcut;Designer.shortcuts.backspace=Designer.deleteShortcut;Designer.shortcuts["ctrl+d"]=Designer.stage.selection.duplicate;Designer.shortcuts["meta+d"]=Designer.stage.selection.duplicate;
Designer.editSourceToggle=function(){$(".selection").length?Designer.stage.selection.editSource():Designer.stage.editSource()};Designer.shortcuts["ctrl+u"]=Designer.editSourceToggle;Designer.shortcuts["meta+u"]=Designer.editSourceToggle;Designer.shortcuts["meta+shift+u"]=Designer.codePanel.toggle;Designer.shortcuts["meta+e"]=Designer.stage.selection.editProperty;Designer.shortcuts["meta+l"]=Designer.stage.selection.editLoop;
Designer.shortcuts["shift+space"]=function(){var a=prompt("Enter a command");if(!a)return!1;a.match(/^(w|width) (.*)/)&&(a=a.match(/^(w|width) (.*)/),Designer.stage.selection.css("width "+a[2]))};Designer.shortcuts["meta+backspace"]=Designer.menu.delete;Designer.shortcuts["meta+o"]=Designer.menu.spotlight;Designer.shortcuts["ctrl+o"]=Designer.menu.spotlight;Designer.shortcuts["ctrl+n"]=Designer.menu.blank;Designer.shortcuts["meta+n"]=Designer.menu.blank;Designer.shortcuts.esc=Designer.stage.selection.clear;
Designer.shortcuts["shift+n"]=Designer.menu.duplicate;Designer.shortcuts.up=function(){Designer.stage.selection.move(0,-1)};Designer.shortcuts.left=function(){Designer.stage.selection.move(-1,0)};Designer.shortcuts.down=function(){Designer.stage.selection.move(0,1)};Designer.shortcuts.right=function(){Designer.stage.selection.move(1,0)};Designer.shortcuts["shift+up"]=function(){Designer.stage.selection.move(0,-10)};Designer.shortcuts["shift+left"]=function(){Designer.stage.selection.move(-10,0)};
...
Designer.shortcuts["meta+shift+s"]=Designer.stage.selection.cssPrompt;Designer.realtime=function(a,b){var d=a.split(/ /g);if("timelines"!==d[0]||d[1]!==Designer.stage.activePage)return!0;Designer.stage.isBehind();b.get("values");Designer.trigger("step");Designer.stage.redo()};Designer.on("close",function(){Project.off("incoming-append",Designer.realtime)});Designer.on("open",function(){Project.on("incoming-append",Designer.realtime)});var Files=new Tool("Files");Files.set("color","rgba(36, 65, 112, 1)");
Files.set("path","");Files.set("description","View and edit the files of your project.");Files.on("open",function(){$("#FilesDownload").attr("href","/nudgepad.backup/"+document.location.host+".zip");Files.refresh()});Files.on("set",function(a){"path"===a&&Files.renderExplorer()});Files.isImage=function(a){return a.match(/\.png|jpeg|jpg|gif$/i)?!0:!1};Files.newFile=function(){var a=prompt("Enter the filename");if(!a)return!1;var b=Files.get("path")?Files.get("path")+" ":"";Explorer.create(b+a,"",Files.refresh)};
Files.newFolder=function(){var a=prompt("Enter the folder name");if(!a)return!1;var b=Files.get("path")?Files.get("path")+" ":"";Explorer.mkdir(b+a,Files.refresh)};
