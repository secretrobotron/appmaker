(function(a,b){function d(a){var b=a.length,d=h.type(a);return h.isWindow(a)?!1:1===a.nodeType&&b?!0:"array"===d||"function"!==d&&(0===b||"number"==typeof b&&0<b&&b-1 in a)}function c(a,d,c,e){if(h.acceptData(a)){var f,g,p=h.expando,j="string"==typeof d,k=a.nodeType,q=k?h.cache:a,u=k?a[p]:a[p]&&p;if(u&&q[u]&&(e||q[u].data)||!j||c!==b)return u||(k?a[p]=u=ya.pop()||h.guid++:u=p),q[u]||(q[u]={},k||(q[u].toJSON=h.noop)),("object"==typeof d||"function"==typeof d)&&(e?q[u]=h.extend(q[u],d):q[u].data=h.extend(q[u].data,
d)),f=q[u],e||(f.data||(f.data={}),f=f.data),c!==b&&(f[h.camelCase(d)]=c),j?(g=f[d],null==g&&(g=f[h.camelCase(d)])):g=f,g}}function e(a,b,d){if(h.acceptData(a)){var c,e,f,g=a.nodeType,p=g?h.cache:a,j=g?a[h.expando]:h.expando;if(p[j]){if(b&&(f=d?p[j]:p[j].data)){h.isArray(b)?b=b.concat(h.map(b,h.camelCase)):b in f?b=[b]:(b=h.camelCase(b),b=b in f?[b]:b.split(" "));c=0;for(e=b.length;e>c;c++)delete f[b[c]];if(!(d?k:h.isEmptyObject)(f))return}(d||(delete p[j].data,k(p[j])))&&(g?h.cleanData([a],!0):h.support.deleteExpando||
p!=p.window?delete p[j]:p[j]=null)}}}function f(a,d,c){if(c===b&&1===a.nodeType){var e="data-"+d.replace(sa,"-$1").toLowerCase();if(c=a.getAttribute(e),"string"==typeof c){try{c="true"===c?!0:"false"===c?!1:"null"===c?null:+c+""===c?+c:Fa.test(c)?h.parseJSON(c):c}catch(f){}h.data(a,d,c)}else c=b}return c}function k(a){for(var b in a)if(("data"!==b||!h.isEmptyObject(a[b]))&&"toJSON"!==b)return!1;return!0}function g(){return!0}function j(){return!1}function m(a,b){do a=a[b];while(a&&1!==a.nodeType);
return a}function s(a,b,c){if(b=b||0,h.isFunction(b))return h.grep(a,function(a,Q){return!!b.call(a,Q,a)===c});if(b.nodeType)return h.grep(a,function(a){return a===b===c});if("string"==typeof b){var d=h.grep(a,function(a){return 1===a.nodeType});if(Rc.test(b))return h.filter(b,d,!c);b=h.filter(b,d)}return h.grep(a,function(a){return 0<=h.inArray(a,b)===c})}function w(a){var b=qc.split("|");a=a.createDocumentFragment();if(a.createElement)for(;b.length;)a.createElement(b.pop());return a}function v(a){var b=
a.getAttributeNode("type");return a.type=(b&&b.specified)+"/"+a.type,a}function r(a){var b=Sc.exec(a.type);return b?a.type=b[1]:a.removeAttribute("type"),a}function F(a,b){for(var c,d=0;null!=(c=a[d]);d++)h._data(c,"globalEval",!b||h._data(b[d],"globalEval"))}function A(a,b){if(1===b.nodeType&&h.hasData(a)){var c,d,e;d=h._data(a);var f=h._data(b,d),g=d.events;if(g)for(c in delete f.handle,f.events={},g){d=0;for(e=g[c].length;e>d;d++)h.event.add(b,c,g[c][d])}f.data&&(f.data=h.extend({},f.data))}}function n(a,
...
Blog.install=function(){Project.get("posts")||Explorer.mkdir("private posts")};Blog.permalink=function(a){return!a?"":a.toLowerCase().replace(/[^a-z0-9- _\.]/gi,"").replace(/ /g,"-")};Blog.pressPost=function(a,b){var d=new Space(a),c=(new Page(b)).toHtml(),c=c.replace(/Blog Post Title/g,d.get("title")),c=c.replace("Blog Post Content",d.get("content")),d=parseFloat(d.get("timestamp")),d=moment(d).format("MMMM Do YYYY, h:mm:ss a"),c=c.replace(/Blog Post Date/g,d);return c=html_beautify(c)};
