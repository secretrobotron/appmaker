...
AppMaker.selectText=function(a){if(document.selection){var b=document.body.createTextRange();b.moveToElementText(document.getElementById(a));b.select()}else window.getSelection&&(b=document.createRange(),b.selectNode(document.getElementById(a)),window.getSelection().addRange(b))};
AppMaker.qrCode=function(){ImagePreview('<div id="qrCode"><div id="AppMakerManifestLink">'+("http://"+document.location.host+"/manifest.webapp")+"</div>");new QRCode("qrCode",{text:"http://"+document.location.host+"/",width:512,height:512,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H})};AppMaker.shortcuts={};AppMaker.shortcuts.backspace=function(){$(".ui-selected").remove()};AppMaker.shortcuts["delete"]=function(){$(".ui-selected").remove()};var Blog=new Tool("Blog");
Blog.set("color","rgba(224, 54, 52, 1)");Blog.set("description","Add a blog to your project.");Blog.set("beta","true");Blog.activePost=null;Blog.createPost=function(){$("#BlogContent,#BlogTitle").val("");$("#BlogAdvanced").val("timestamp "+(new Date).getTime()+"\ntemplate blog");$("#BlogPermalink").attr("value","");$("#BlogTitle").focus();Blog.activePost=null};
...
j=Boston.StretchHandle.getLength(e,f,k,g,"center"!=d,"middle"!=b),k=a.scrap(),g=k.get("style width")||a.css("width"),n=k.get("style height")||a.css("height");"center"!==d&&(k.set("style left",(0<=j.x?e:j.x+e)+"px"),g=a.outerWidth()-a.width(),g=Math.abs(j.x)-g,g=Boston.StretchHandle.isWidthPercentage?Math.round(100*g/a.parent().width())+"%":g+"px",k.set("style width",g));c?(b=parseFloat(k.get("style width"))/Boston.StretchHandle.originalWidth,k.set("style height",Math.round(Boston.StretchHandle.originalHeight*
b)+"px")):"middle"!=b&&(0<=j.y?k.set("style top",f+"px"):k.set("style top",j.y+f+"px"),b=a.outerHeight(!0)-a.height(),n=Math.abs(j.y)-b,b=a.parent().height()||$(window).height(),n=Boston.StretchHandle.isHeightPercentage?Math.round(100*n/b)+"%":n+"px",k.set("style height",n));a.css(k.values.style.values);$("#BostonDimensions").css({left:10+a.offset().left+a.outerWidth(),top:-10+a.offset().top+Math.round(a.outerHeight(!0)/2)}).html("W "+g+"<br> H "+n);$(".stretchHandle"+k.id).trigger("update");return!1};
...
Boston.styleEditor.edit=function(a){$(".handle").remove();var b=$('<div id="BostonStyleEditorModal"/>');b.on("tap mousedown click slide slidestart slideend mouseup",function(a){a.stopPropagation()});b.on("click",function(){w.trigger("click")});$("body").append(b);var d=a.element(),c=$('<div id="BostonStyleEditor"></div>');c.css({left:d.offset().left+2+"px",top:d.offset().top+d.outerHeight()+4+"px"});d=$('<div id="BostonStyleEditorTopBar"></div>');c.append(d);var e=$('<div class="editorOption selectedEditorOption">Block</div>');
...
d.on("tap",function(){Boston.stage.selection.patch("locked true");w.trigger("click");return!1});f.append(d);var g=$('<select id="nudgepadLinkSelect"></select>'),d=$("<option>No link</option>");d.on("click",function(){Boston.stage.selection.patch("href ")});g.append(d);_.each(Project.values.pages.values,function(a,b){var c=$('<option value="'+b+'">'+ToProperCase(b)+"</option>");c.on("click",function(){Boston.stage.selection.patch("tag a\nhref "+$(this).attr("value"))});g.append(c)});d=$("<option>New Page</option>");
d.on("click",function(){var a=prompt("Enter the name of your new page",Boston.menu.nextName(Boston.stage.activePage));if(a){a=Permalink(a);Boston.stage.selection.patch("tag a\nhref "+a);w.trigger("click");var b=Boston.stage.activePage;Boston.menu.duplicate(null,a,!0);Boston.stage.open(b)}});g.append(d);d=$("<option>External Link</option>");d.on("click",function(){var a=prompt("Enter the url to link to","http://");a&&Boston.stage.selection.patch("tag a\nhref "+a)});g.append(d);g.css({display:"none",
...
return!1});e.append(j);j=$('<div id="textStyleThree" class="toolButton toolButtonThree">Auto</div>');j.on("click",function(){Boston.stage.selection.cssPreview({"text-decoration":"none","font-weight":"normal","font-style":"normal"});return!1});e.append(j);j=$('<div class="clear"></div>');s.append(j);var p=$('<textarea id="BostonStyleEditorCssEditor"></textarea>');a.get("style")&&p.val(a.get("style"));p.on("tap mousedown click slide slidestart slideend mouseup",function(a){a.stopPropagation()});Boston.styleEditor.livePreviewScrap=
a;p.on("keyup",Boston.styleEditor.livePreviewStart);c.append(p);e=$('<div id="BostonStyleEditorBottomBar"></div>');p.append(e);var w=$('<div id="BostonStyleEditorSaveButton">Save</div>');w.on("click",function(){Boston.stage.commit();$(".handle").trigger("update");c.remove();b.remove()});p.append(w);$("body").append(c);var x;$(".colorPicker").spectrum({color:k,move:function(a){Boston.stage.selection.cssPreview("background-color "+a.toHexString());k=a.toHexString()},change:function(a){$("#basic-log").text("change called: "+
a.toHexString());k=a.toHexString()},show:function(){x="background-color "},hide:function(){Boston.stage.commit();$(".handle").trigger("update")}});$(".colorBorderPicker").spectrum({color:n,move:function(a){Boston.stage.selection.cssPreview("border-color "+a.toHexString());n=a.toHexString()},change:function(a){$("#basic-log").text("change called: "+a.toHexString());n=a.toHexString()},show:function(){x="border-color "},hide:function(){Boston.stage.commit();$(".handle").trigger("update")}});$(".colorFontPicker").spectrum({color:d,
move:function(a){Boston.stage.selection.cssPreview("color "+a.toHexString());n=a.toHexString()},change:function(a){$("#basic-log").text("change called: "+a.toHexString());n=a.toHexString()},show:function(){x="color "},hide:function(){Boston.stage.commit();$(".handle").trigger("update")}});$(document).on("slide slidestart slideend",".sp-container",function(){return!1});$(".sp-input[type=text]").on("keydown",function(a){if(13==a.which)return a.preventDefault(),a=$(this).val(),Boston.stage.selection.css(x+
...
$(":input[name=fontFamilyDropdown] option").each(function(a,b){$(b).text()==r&&$(this).attr("selected","selected")});d=c.position().top+c.outerHeight()-Boston.stage.height();0<d&&($("#BostonStage").scrollTop($("#BostonStage").scrollTop()+d),c.css("top",parseFloat(c.css("top"))-d));d=c.position().left+c.outerWidth()-$(window).width();0<d&&($("#BostonStage").scrollLeft($("#BostonStage").scrollLeft()+d),c.css("left",parseFloat(c.css("left"))-d))};
